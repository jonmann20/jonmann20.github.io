...
class StarryBg {
	constructor(doc) {
		...
		const color = doc.querySelector('input[type=radio]:checked').value;
		this.initStar(color);
		...
		// doc.getElementById('custom').addEventListener('change', this.onColorChange);
	}
	...
	destroy(page) {
		...
		let radios = Array.from(this.doc.querySelectorAll('input[type=radio]'));
		for(let radio of radios) {
			radio.removeEventListener('click', e => this.onColorChange(e.target.value), {passive: true});
		}

		//this.doc.getElementById('custom').removeEventListener('change', this.onColorChange);
		delete window.starryBg;
	}

	initStar(color) {
		this.starBg = new StarBg({
			elt: this.doc.getElementById('starry-canvas'),
