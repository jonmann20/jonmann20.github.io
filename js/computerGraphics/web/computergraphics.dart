...
class Simulation{
  ...
  var canvas = document.querySelector("canvas");//query('canvas');
  var ctx;
  ...
  Simulation(){
    ...
    FULLW = canvas.width = 600;//query('.main').client.width / 1.5;
  ...
  void init(){
    ...
    Element canvasListner = query('canvas');

    canvasListner.onMouseDown.listen((e){
    ...
    canvasListner.onMouseMove.listen((e){
          if(mouseDown){
              //print("${canvasListner.offset.left}, ${canvasListner.offset.top}");
              //print("client: ${e.client.x}, ${e.client.y}");

              mouse_x = e.client.x - canvasListner.offset.left;
              mouse_y = e.client.y - canvasListner.offset.top;

              //print((sun.x - sun.oldX)/dt + ',' + dt);
      ...
      // page elts
      InputElement slider = query('.diffusionRate');

      slider.onChange.listen((e){
        diffuse = slider.value;

        query('.litDiffusion').innerHtml = diffuse.toString();
      });
