...
Enemy: {"": "GameObj;initHealth,health,invincibleTimer,invincible,captured,imgCardReady?,animate,cardSrc,name,imgCard,hdir,vdir,ix,iy,w,h,x,y,ready,img,src",
  ...
  Enemy$8: function(initHealth, cardSrc, $name, ix, iy, w, h, src) {
    ...
    $.set$src$x(this.imgCard, "img/" + this.cardSrc);
...
Arena: {"": "Object;curStage,numStages,animalSelected,stage,stageSelected",
  ...
  drawStage1$0: function() {
    ...
    if (t1 !== -1) {
      ...
      if (t4 >>> 0 !== t4 || t4 >= t3.length)
        throw $.ioore(t4);
      t1.drawD$2(t2 - t3[t4].w / 2, $.FULLH - 120);
    } else
...
Level: {"": "Object;startPositions,level1,arena,litLvlName,bgColor,isCutscene",
  ...
  Level$0: function() {
    ...
    if (0 >= t3.length)
      throw $.ioore(0);
    t1.x = $.$sub$n($.$add$ns(t3[0].x, t2.boxW / 2), $.p.w / 2);
    t2 = this.startPositions;
    if (0 >= t2.length)
      throw $.ioore(0);
    t2 = t2[0];
    ...
    if (t4 >>> 0 !== t4 || t4 >= t1.length)
      throw $.ioore(t4);
    t2.y = $.$sub$n($.$add$ns(t1[t4].y, t3.boxH / 2), $.p.h);
    ...
    t1 = $.p;
    t3.x = t4 - t1.w / 2;
    t4 = this.startPositions;
    if (1 >= t4.length)
      throw $.ioore(1);
    t4[1].y = $.FULLH - t1.h - 10;
    t1 = this.startPositions;
    if (1 >= t1.length)
      throw $.ioore(1);
    t1[1].dir = 38;
    ...
    if (2 >= t1.length)
      throw $.ioore(2);
    t1[2].x = $.get$HALFW() - 70 + $.p.w / 2;
    t1 = this.startPositions;
    if (2 >= t1.length)
      throw $.ioore(2);
    t1[2].y = $.get$HALFH() - 55 - $.p.h / 2;
    t1 = this.startPositions;
    if (2 >= t1.length)
      throw $.ioore(2);
    t1[2].dir = 40;
      ...
      if (i !== $.NUM_LVLS - 1) {
        ...
        if (t1) {
          if (t4 >= t2.length)
            throw $.ioore(t4);
          t1 = t2[t4];
          t3.lineTo(t1.x, $.$add$ns(t1.y, this.boxH / 2));
        } else {
          if (t4 >= t2.length)
            throw $.ioore(t4);
          t1 = $.$add$ns(t2[t4].x, this.boxW / 2);
          t2 = this.lvlArr;
          if (i >= t2.length)
            throw $.ioore(i);
          t3.lineTo(t1, $.$add$ns(t2[i].y, this.boxH / 2));
          t2 = $.ctx;
          t1 = this.lvlArr;
          if (t4 >= t1.length)
            throw $.ioore(t4);
          ...
          if (i >= t3.length)
            throw $.ioore(i);
          t2.moveTo(t1, $.$add$ns(t3[i].y, this.boxH / 2));
          t3 = $.ctx;
          t1 = this.lvlArr;
          if (t4 >= t1.length)
            throw $.ioore(t4);
          ...
          if (t4 >= t2.length)
            throw $.ioore(t4);
          t3.lineTo(t1, $.$add$ns(t2[t4].y, this.boxH / 2));
        }
        $.ctx.stroke();
        $.ctx.closePath();
      }
...
Player: {"": "GameObj;vX,vY,maxVx,maxVy,maxV,health,maxHealth,invincibleTimer,atkDownTime,atkUpTime,prevDir,dir,movLocked,invincible,invisible,heart,device,sword,animal,imgD,imgL,imgR,imgU,imgPicked,ix,iy,w,h,x,y,ready,img,src",
  update$0: function() {
    ...
    if (!this.movLocked) {
      ...
      t1 = $.key;
      t1.isPressed$1;
      if (t1._liblib3$_keys.containsKey$1(32))
        ;
      t1 = this.sword;
      if (t1.usable) {
        t2 = $.key;
        t2.isPressed$1;
        t2 = t2._liblib3$_keys.containsKey$1(32);
      } else
        t2 = false;
      t1.inUse = t2 && true;
      t1 = this.device;
      if (t1.usable) {
        t2 = $.key;
        t2.isPressed$1;
        t2 = t2._liblib3$_keys.containsKey$1(16);
      } else
        t2 = false;
      t1.inUse = t2 && true;
    }
  ...
  updatePosition$0: function() {
    ...
    if (!tooFastY) {
      t1 = $.key;
      t1.isPressed$1;
      if (t1._liblib3$_keys.containsKey$1(38)) {
        this.vY = this.vY - 1;
        this.dir = 38;
      }
      t1 = $.key;
      t1.isPressed$1;
      if (t1._liblib3$_keys.containsKey$1(40)) {
        this.vY = this.vY + 1;
        this.dir = 40;
      }
    }
    if (!tooFastX) {
      t1 = $.key;
      t1.isPressed$1;
      if (t1._liblib3$_keys.containsKey$1(37)) {
        this.vX = this.vX - 1;
        this.dir = 37;
      }
      t1 = $.key;
      t1.isPressed$1;
      if (t1._liblib3$_keys.containsKey$1(39)) {
        this.vX = this.vX + 1;
        this.dir = 39;
      }
    }
    ...
    if (t1._liblib3$_keys.containsKey$1(38)) {
      t1 = $.key;
      t1.isPressed$1;
      t1 = t1._liblib3$_keys.containsKey$1(37);
    } else
      t1 = false;
    if (!t1) {
      t1 = $.key;
      ...
      if (t1._liblib3$_keys.containsKey$1(38)) {
        t1 = $.key;
        t1.isPressed$1;
        t1 = t1._liblib3$_keys.containsKey$1(39);
      } else
        t1 = false;
      if (!t1) {
        t1 = $.key;
...
StartScreen: {"": "Object;isOptions,linkClicked?,ret,ent,OPEN_GAME,OPEN_START,mouse<,newGame<,linkColor?,mouseMoveE,mouseClickE",
  ...
  startScreen$0: function() {
    var t1 = $.key;
    t1.isPressed$1;
    if (t1._liblib3$_keys.containsKey$1(13) || $.DEBUG_OVERWORLD || $.DEBUG_LVL) {
      $.key.lastKeyDown = 8;
      this.mouseEvents$0();
      return true;
    }
...
StartScreen_mouseEvents_closure: {"": "Closure;this_0",
  call$1: function(e) {
    ...
    $.Primitives_printString("mouse: (" + $.S(t1.get$mouse().x) + ", " + $.S(t1.get$mouse().y) + ")");
...
["metadata", "../../../../../Programs/dart/dart-sdk/lib/html/html_common/metadata.dart", , {
